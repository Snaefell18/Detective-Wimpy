...
import { AUTO_GROESSE, AUTO_MODELLE, autoGroesse, autoGueltig, autoLaenge, START_AUTO_ID, type Auto } from '@/lib/autos';
...
export function AutosBereich({ onMeldung, onFehler }: BereichProps) {
  ...
  const neu = () => setEntwurf({ id: `auto-${crypto.randomUUID()}`, name: 'Neues Auto', modell: AUTO_MODELLE[0]?.id ?? '', speed: 160, beschleunigung: 30, drehung: 0, groesse: 1, preis: 800, bild: '', beschreibung: '', wirkung: 'auto', erstelltAm: Date.now() });
  const speichern = async () => {
    if (!entwurf || !autoGueltig(entwurf)) { onFehler('Bitte Modell, Namen und gültige Fahrwerte eingeben.'); return; }
    setSpeichert(true);
    try {
      await speichereZubehoer({ ...entwurf, groesse: autoGroesse(entwurf), preis: entwurf.id === START_AUTO_ID ? 0 : Math.round(entwurf.preis), versteckt: entwurf.id === START_AUTO_ID ? false : entwurf.versteckt ?? false });
  ...
  return <><h2>Autos</h2><p className="leise">Wimpys Garage · Speed in km/h, Beschleunigung in km/h pro Sekunde. Der kostenlose Startwagen hält bestehende Sagas spielbar.</p>
    ...
    {autos.map(auto => <div className="kapitel-block" key={auto.id}><strong>{auto.name}</strong><p>{auto.speed} km/h · Beschleunigung {auto.beschleunigung} · ¥{auto.preis} · {autoGroesse(auto).toFixed(2).replace('.', ',')}× groß</p><button className="knopf" onClick={() => setEntwurf(auto)}>Bearbeiten</button></div>)}
    {entwurf && <div className="kapitel-block">
      ...
      {/*
        * Die Größe: Jedes Modell wird im Spiel auf dieselbe Länge gebracht -
        * und genau das macht aus einer Limousine ein Spielzeugauto. Hier
        * darf sie größer sein, und die Vorschau zeigt es sofort: Die
        * Fahrbahn bleibt gleich breit, der Wagen nicht.
        */}
      <label className="feld">
        Größe · {autoGroesse(entwurf).toFixed(2).replace('.', ',')}× ·
        {' '}ca. {autoLaenge(entwurf, 3.5).toFixed(1).replace('.', ',')} m lang
        <input
          type="range"
          min={AUTO_GROESSE.min}
          max={AUTO_GROESSE.max}
          step={AUTO_GROESSE.schritt}
          value={autoGroesse(entwurf)}
          onChange={e => setEntwurf({ ...entwurf, groesse: Number(e.target.value) })}
        />
      </label>
      <p className="leise klein">1,0× ist ein gewöhnlicher Wagen (3,5 m). Eine Limousine steht bei 1,4× bis 1,6×, ein Kleinwagen bei 0,8×.</p>
      {/* Statt einer geratenen Gradzahl: hinsehen. */}
      <span className="leise klein">Ausrichtung · {entwurf.drehung}°</span>
      <AutoVorschau modell={entwurf.modell} drehung={entwurf.drehung} groesse={autoGroesse(entwurf)} onDrehen={grad => setEntwurf({ ...entwurf, drehung: grad })} />
