...
import { abspannZeile } from "@/lib/abspann";
...
import { STANDARD_KAMPF, kampfZeile } from "@/lib/endkampf";
...
export function SagaVorgabenFelder({
  ...
  /*
   * Das Formular ist lang, und das Meiste daran ist freiwillig. Damit man
   * den Stand sieht, ohne alles aufzuklappen, trägt jeder Abschnitt seine
   * Einstellungen in einer Zeile mit sich - dieselben Werte, nur kurz.
   */
  const undZaehlen = (teile: (string | false | null | undefined)[]) =>
    teile.filter(Boolean).join(" · ");

  const standSaga = undZaehlen([
    vorgaben.name.trim() || "noch ohne Namen",
    vorgaben.stadt === "zufall" ? "Stadt: Zufall" : vorgaben.stadt,
    `${vorgaben.ortsAnzahl} Schauplätze`,
  ]);
  const standKapitel = undZaehlen([
    `${vorgaben.kapitelAnzahl} Kapitel + Finale`,
    (vorgaben.versammlungen?.length ?? 0) > 0 &&
      `${vorgaben.versammlungen?.length} Versammlung${vorgaben.versammlungen?.length === 1 ? "" : "en"}`,
    (vorgaben.verfolgungsjagden?.length ?? 0) > 0 &&
      `${vorgaben.verfolgungsjagden?.length} Verfolgungsjagd${vorgaben.verfolgungsjagden?.length === 1 ? "" : "en"}`,
  ]);
  const standBesetzung = undZaehlen([
    vorgaben.charaktere.length ? `${vorgaben.charaktere.length} Tiere` : "alle Tiere",
    vorgaben.items.length > 0 && `${vorgaben.items.length} Dinge`,
    vorgaben.drahtzieherId
      ? `Drahtzieher: ${namenVon(vorgaben.drahtzieherId)}`
      : "Drahtzieher: zufällig",
    vorgaben.twist && "mit Twist",
  ]);
  const standTon = undZaehlen([
    REIFEGRADE.find((r) => r.id === vorgaben.reifegrad)?.label,
    TOENE.find((t) => t.id === vorgaben.ton)?.label,
    SCHWIERIGKEITEN.find((s) => s.id === vorgaben.schwierigkeit)?.label,
  ]);
  const standFinale = undZaehlen([
    FINALE_ARTEN.find((e) => e.id === art)?.label,
    mitKampf(art) && kampfZeile(vorgaben.kampf ?? STANDARD_KAMPF),
    besessen(vorgaben) && "mit Besessenheit",
  ]);
  /*
   * Ein Ausrufezeichen dort, wo das Finale noch etwas braucht: eine
   * Besessenheit ohne Gestalt hat keinen Grund, und ein Gerichtsfinale mit
   * Dämon steht ohne sie still.
   */
  const finaleHinweis =
    besessen(vorgaben) && !vorgaben.besessenheit?.daemonId ? "Gestalt fehlt" : undefined;
  const standAbspann = abspannZeile(vorgaben.abspann);

  /** Und dieselbe Zeile je Kapitel: was darin schon eingetragen ist. */
  const standKapitelblock = (i: number, istFinale: boolean) => {
    const taeter = istFinale ? vorgaben.drahtzieherId : vorgaben.kapitelTaeter?.[i];
    const dreiD = vorgaben.kapitel3d?.[i]?.aktiv;
    return undZaehlen([
      (vorgaben.kapitelWuensche?.[i] ?? "").trim() ? "mit Wunsch" : null,
      istFinale
        ? null
        : taeter
      ...
      (vorgaben.kapitelGeschenke?.[i] ?? "").trim() ? "Geschenk" : null,
  ...
  return (
    <>
      <Abschnitt titel="Die Saga" zusammenfassung={standSaga} offen>
        ...
        <h3 className="unter-abschnitt">
          Stadt{" "}
          <span className="leise">
            · gilt, wo bei den Kapiteln „Wie eingestellt“ steht
          </span>
        </h3>
      ...
      <Abschnitt titel="Kapitel" zusammenfassung={standKapitel}>
        <h3 className="unter-abschnitt">Wie viele Kapitel?</h3>
        <div className="wahl-reihe">
          {[2, 3, 4, 5, 6, 7, 8].map((n) => (
            <button
              key={n}
        ...
        {[...kapitelNummern, vorgaben.kapitelAnzahl].map((i) => {
          ...
          /*
           * Jedes Kapitel klappt für sich zu. Acht Kapitel mit Wunsch, Täter,
           * Video, Musik, Geschenk, Wetter, Stadt und 3D-Welt sind sonst eine
           * Bahn, auf der man das dritte nur findet, indem man am zweiten
           * vorbeiscrollt - und in der Kopfzeile steht ohnehin schon, was
           * darin eingestellt ist.
           */
          return (
            <Abschnitt
              key={i}
              titel={istFinale ? "Finale" : `Kapitel ${i + 1}`}
              zusammenfassung={standKapitelblock(i, istFinale)}
            >
      ...
      <Abschnitt titel="Besetzung" zusammenfassung={standBesetzung}>
        <h3 className="unter-abschnitt">
      ...
      <Abschnitt titel="Ton und Publikum" zusammenfassung={standTon}>
      ...
      <Abschnitt titel="Finale" zusammenfassung={standFinale} hinweis={finaleHinweis}>
        <h3 className="unter-abschnitt">
      ...
      <Abschnitt titel="Abspann" zusammenfassung={standAbspann}>
        {/*
