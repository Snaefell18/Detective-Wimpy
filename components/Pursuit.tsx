...
import { PursuitShowdown } from './PursuitShowdown';
...
export function Pursuit({ onSchliessen }: { onSchliessen: () => void }) {
  ...
  if (modus === 'showdown') return <PursuitShowdown onZurueck={zurueck} onSchliessen={onSchliessen} />;
  ...
  return <div className="jagd pursuit-auswahl pursuit-spiel pursuit-modi"><button className="jagd-vorschau-schliessen" onClick={onSchliessen} aria-label="Pursuit schließen">×</button><section className="pursuit-panel"><span className="jagd-kicker">WÄHLE DEIN CHAOS</span><h1>PURSUIT</h1><div className="pursuit-moduswahl">
    {/* Der Showdown steht oben: Er ist das Neueste hier, und auf einem
        Handy stehen die Karten untereinander - was hinten steht, findet
        niemand. */}
    {[
      ['showdown', 'MODUS V · NEU', 'SHOWDOWN', 'Bau eine Arena und kämpfe gegen den Gegner deiner Wahl - mit allen Einstellungen aus dem Finale.'],
    ...
    ].map(([id, kicker, name, text]) => <button className="pursuit-moduskarte" data-modus={id} key={id} onClick={() => setModus(id)}><small>{kicker}</small><strong>{name}</strong><span>{text}</span><b>STARTEN ›</b></button>)}
  </div></section></div>;
}
